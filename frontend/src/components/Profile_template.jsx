...
import { FaEnvelope, FaPhone, FaMapMarkerAlt, FaBriefcase, FaCode, FaUserCircle } from 'react-icons/fa';
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            try {
                ...
                if (!token) {
                    console.error('Authentication token not found. Cannot fetch profile.');
                    setError('You need to be logged in to view profiles.');
                    setLoading(false);
                    return;
                }
                if (!userId) {
                    console.error('User ID not provided to Profile_template.');
    ...
    return (
        ...
        >
            ...
            >
                ...
                {(user.position || user.company) && (
                    <p className="text-xl text-gray-600 mt-2 flex items-center gap-2">
                        ...
                        {user.position && user.company ? `${user.position} at ${user.company}` : user.position || user.company}
                    </p>
                )}
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <motion.div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl shadow-md border border-blue-100" variants={itemVariants}>
                    <h3 className="text-2xl font-bold text-[#333] mb-4 flex items-center gap-3">
                ...
                <motion.div className="bg-gradient-to-l from-green-50 to-emerald-50 p-6 rounded-xl shadow-md border border-green-100" variants={itemVariants}>
                    <h3 className="text-2xl font-bold text-[#333] mb-4 flex items-center gap-3">
